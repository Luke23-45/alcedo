...
export function ProgressionRulesEditor(props: Props) {
  ...
  return (
    <View style={{ gap: theme.space.sm }} testID="progression-rules">
      {rules.length === 0 && (
        <Text variant="bodySmall" style={{ color: theme.color.content.secondary }}>
          {t('exercise.progression.none.body')}
        </Text>
      )}
      {rules.map((rule, index) => (
        <View key={index} style={deadFrom !== undefined && index >= deadFrom ? { opacity: 0.5 } : undefined}>
          {index > 0 && <Divider style={{ marginBlock: theme.space.sm }} />}
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Text variant="labelLarge" style={{ flex: 1, color: theme.color.content.secondary }}>
      ...
      {rules.length > 0 && <Divider style={{ marginBlockStart: theme.space.sm }} />}
  ...
  return (
    <View>
      ...
      {rule.axis === 'load' && !props.canMoveLoad && (
        <Text variant="bodySmall" style={{ color: theme.color.status.danger.base }}>
...
function numberInputStyle(theme: AppTheme) {
  return {
    dense: true,
    textColor: theme.color.interactive.tint,
...
function SwitchRow(props: { label: string; description: string; value: boolean; onValueChange: (v: boolean) => void }) {
  ...
  return (
    <TouchableRipple onPress={() => props.onValueChange(!props.value)}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: theme.space.base, paddingBlock: theme.space.sm }}>
      ...
      <Portal>
        <Dialog visible={exampleOpen} onDismiss={() => setExampleOpen(false)}>
          ...
          <Dialog.Content style={{ height: 400 }}>
            <ScrollView contentContainerStyle={{ gap: theme.space.base, alignItems: 'center' }} style={{ flex: 1 }}>
