...
export function ActivityCalendar({
  ...
  return (
    <View style={{ alignItems: 'stretch', gap: theme.space.xs }}>
      {header}

      <View style={{ flexDirection: 'row', alignItems: 'center', gap: theme.space.xs }}>
        {isWeek && <View style={{ width: LABEL_WIDTH }} />}
        <ForceLTRRow style={{ flex: 1, gap: theme.space.xs }}>
      ...
      {rows.map((row, rowIndex) => {
        ...
        if (isWeek) {
          return (
            <View key={row.key} style={{ flexDirection: 'row', alignItems: 'center', gap: theme.space.xs }}>
        ...
        return (
          <View key={row.key} style={{ gap: theme.space.xs }}>
            {cells}
          </View>
        );
      })}
