...
/**
 * Reference chart geometry (dark SVG + light deltas):
...
export function WeeklyVolume({ data }: { data: WeeklyVolumeData }) {
  ...
  const chartW = FIRST_X * 2 + (COLS - 1) * PITCH + BAR_W;
  const chartH = TRACK_H + LABEL_H;

  return (
    <HomeCard style={{ height: 162 }}>
      <S.HeaderRow>
        <HomeText
          weight={fontWeight.semibold}
          tracking={-0.3}
          style={{ fontSize: 15.5, lineHeight: 19, color: dark ? '#FFFFFF' : '#1C1C1E' }}
        >
      ...
      >
        {
          t('home.volume.subtitle', {
            total: Math.round(data.totalKg).toLocaleString('en-US'),
            ...
            {data.days.map((day, i) => {
              ...
              return (
                <G key={`${day.label}-${i}`}>
                  ...
                  {day.isToday && (
                    <>
                      <Rect x={x} y={y} width={BAR_W} height={Math.min(26, h)} fill="url(#wvGloss)" />
