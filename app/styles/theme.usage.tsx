...
import { createTheme, type, hitSlopFor, alpha, type AppTheme } from './theme';
...
const ButtonSurface = styled.View<{ $variant: ButtonVariant; $size: ButtonSize; $disabled?: boolean }>`
  ...
  background-color: ${({ theme, $variant }) =>
    $variant === 'accent'
      ? theme.color.interactive.accent
      : $variant === 'tint'
        ? alpha(theme.color.interactive.tint, 0.14) // tonal, not solid — see note below
        : $variant === 'subtle'
...
export function Button({
  ...
  const tone: Tone =
    variant === 'accent' ? 'inverse' : variant === 'tint' || variant === 'plain' ? 'tint' : 'primary';
